...
- Extensibility
	- Should have the ability add new APIs to the gateway with minimal changes to any existing code
...
All data is only loaded once. To get latest articles, you would need to restart the application. It is also worth mentioning that this may or may not run properly on a machine running an OS other than Mac OS X. I have not tested it on Windows or Linux and am not entirely sure if the app will clear the console properly.
